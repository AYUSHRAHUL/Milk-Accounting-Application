...
export default function ExpensesHistoryScreen() {
    ...
    const [entries, setEntries] = useState<ExpenseEntryData[]>([]);
    ...
    const renderEntry = ({ item }: { item: ExpenseEntryData }) => (
        <View style={[styles.card, { backgroundColor: theme.card, shadowColor: theme.textSecondary }]}>
            <View style={styles.cardHeader}>
                <View style={{ flex: 1, paddingRight: 8 }}>
                    <ThemedText style={{ fontSize: 16, fontWeight: 'bold' }}>{item.category}</ThemedText>
                    <ThemedText style={{ fontSize: 13, color: theme.textSecondary }} numberOfLines={2}>
                        {item.description}
                    </ThemedText>
                </View>
                <View style={{ alignItems: 'flex-end', justifyContent: 'flex-start' }}>
                    <ThemedText style={{ fontSize: 16, fontWeight: 'bold', color: theme.error }}>
                ...
                <FlatList
                    ...
                    ListEmptyComponent={
                        <View style={styles.emptyContainer}>
                            <ThemedText style={{ color: theme.textSecondary, fontSize: 16 }}>No expenses recorded yet.</ThemedText>
                        </View>
                    }
                />
            )}
